...
describe("Find nested tags", () => {
	...
	test("second tag", async () => {
		...
		expect(parserAfter).toEqual("after-bar<foo>inside-foo-2</foo></foo>content-after")
	});
})
...
describe("open pattern matches", () => {
	...
	const checks = [
		...
		{ string: "start <tag attr='value'> end", details: "simple tag with attribute" },
		{ string: 'start <tag attr="value"> end', details: "simple tag with attribute (single-quote)" },
		{ string: 'start <tag attr123="value123"> end', details: "simple tag with attribute (numeric)" },
		{ string: 'start <tag attr_123-foo="value 123-test_"> end', details: "simple tag with attribute (special chars)" }
	]

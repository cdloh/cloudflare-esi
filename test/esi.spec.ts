...
test("TEST 3b: Test comments are removed. Nested.", async () => {
	...
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.end("BEFORE <esi:choose><esi:when test=\"1>2\" ></esi:when><esi:otherwise><esi:remove> FIRST </esi:remove><esi:remove> SECOND </esi:remove></esi:otherwise></esi:choose> AFTER")
	})
	const res = await makeRequest(url);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
...
test("TEST 5: Include fragment", async () => {
	...
	const printFragment = function (req: http.IncomingMessage, res: testResponse) {
	...
	routeHandler.add(url, function (req, res) {
		...
		res.write("3")
		res.end(`<esi:include src="http://localhost:${port}${url}/fragment_1?a=3" />`)
	})
...
test("TEST 5c: Include fragment with absolute URI, schemaless, and no path", async () => {
	...
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.say(`<esi:include src="${testingDetails.proto}//${testingDetails.hostname}:${testingDetails.port}${url}/fragment_1" />`)
		res.say(`<esi:include src="//${testingDetails.hostname}:${testingDetails.port}${url}/fragment_1" />`)
		res.say(`<esi:include src="${testingDetails.proto}//${testingDetails.hostname}:${testingDetails.port}/" />`)
		res.say(`<esi:include src="${testingDetails.proto}//${testingDetails.hostname}:${testingDetails.port}" />`)
		res.end(`<esi:include src="//${testingDetails.hostname}:${testingDetails.port}" />`)
	})
	routeHandler.add(`/`, function (req, res) {
...
test("TEST 6: Include multiple fragments, in correct order.", async () => {
	...
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`FRAGMENT_3\nMID LINE FRAGMENT_1\nFRAGMENT_2\n`);
});
...
test.todo("TEST 7b: Leave instructions intact if ESI delegation is enabled - slow path.");
test.todo("TEST 7c: Leave instructions intact if ESI delegation is enabled - fast path");
test.todo("TEST 7d: Leave instructions intact if ESI delegation is enabled by IP on the slow path");
...
test.todo("TEST 7e: Leave instructions intact if ESI delegation is enabled by IP on the fast path.");
...
test.todo("TEST 7f: Leave instructions intact if allowed types does not match on the fast path.");
...
test("TEST 9: Variable evaluation", async () => {
	...
	routeHandler.add(url, function (req, res) {
		...
		res.say("HTTP_COOKIE{SQ_SYSTEM_SESSION}: <esi:vars>$(HTTP_COOKIE{SQ_SYSTEM_SESSION})</esi:vars>");
		res.say("<esi:vars>");
		res.say("HTTP_COOKIE: $(HTTP_COOKIE)");
		res.say("HTTP_COOKIE{SQ_SYSTEM_SESSION}: $(HTTP_COOKIE{SQ_SYSTEM_SESSION})");
		res.say("HTTP_COOKIE{SQ_SYSTEM_SESSION_TYPO}: $(HTTP_COOKIE{SQ_SYSTEM_SESSION_TYPO}|'default message')");
		res.say("</esi:vars>");
		res.say("<esi:vars>$(HTTP_COOKIE{SQ_SYSTEM_SESSION})</esi:vars>$(HTTP_COOKIE)<esi:vars>$(QUERY_STRING)</esi:vars>")
		res.say("$(HTTP_X_MANY_HEADERS): <esi:vars>$(HTTP_X_MANY_HEADERS)</esi:vars>")
		res.end("$(HTTP_X_MANY_HEADERS{2}): <esi:vars>$(HTTP_X_MANY_HEADERS{2})</esi:vars>")
	})
...
test("TEST 9b: Multiple Variable evaluation", async () => {
	...
	routeHandler.add(`${url}?t=1`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment_1b?$(QUERY_STRING)&test=$(HTTP_X_ESI_TEST)" /> <a href="$(QUERY_STRING)" />`);
	})
	...
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`FRAGMENT: ?t=1&test=foobar <a href="$(QUERY_STRING)" />`);
...
test("TEST 9c: Dictionary variable syntax", async () => {
	...
	routeHandler.add(`${url}?t=1`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment1c?$(QUERY_STRING{t})&test=$(HTTP_COOKIE{foo})" />`);
	})
...
test("TEST 9d: List variable syntax (accept-language)", async () => {
	...
	routeHandler.add(`${url}?t=1`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment1d?$(QUERY_STRING{t})&en-gb=$(HTTP_ACCEPT_LANGUAGE{en-gb})&de=$(HTTP_ACCEPT_LANGUAGE{de})" />`);
	})
	routeHandler.add(`${url}/fragment1d?1&en-gb=true&de=false`, function (req, res) {
		const url = new URL(req.url as string, testingDetails.url);
		res.writeHead(200, esiHead);
		res.end(`FRAGMENT: ${url.search}`);
	})
...
test.todo("TEST 9e: List variable syntax (accept-language) with multiple headers");
...
test.todo("TEST 10d: ESI still runs on cache revalidation, upstream 200, locally valid.");
test.todo("TEST 10e: ESI still runs on cache revalidation, upstream 304, locally valid.");
...
test.todo("TEST 12b: Incomplete ESI tag opening at the end of buffer (lookahead)");
test.todo("TEST 12c: Incomplete ESI tag opening at the end of buffer (lookahead)");
test.todo("TEST 12d: Incomplete ESI tag opening at the end of buffer (lookahead)");
test.todo("TEST 12e: Incomplete ESI tag opening at the end of response (regression)");
...
test("TEST 17: choose - when - test, conditional syntax", async () => {
	...
	routeHandler.add(`/esi/test-17?msg=hello&msg2=hel%27lo`, function (req, res) {
		...
		for (const [, c] of conditions.entries())
			res.say(`<esi:choose><esi:when test="${c}">${c}</esi:when><esi:otherwise>Failed</esi:otherwise></esi:choose>`);
		res.end()
	})
	const res = await makeRequest(url, {
		headers: {
...
test("TEST 22: Test comments are removed.", async () => {
	...
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`1234<esi:comment text="comment text" /> 5678<esi:comment text="comment text 2" />`)
	})
	const res = await makeRequest(url);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
...
test("TEST 22b: Test comments are removed. Nested.", async () => {
	...
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.end("BEFORE <esi:choose><esi:when test=\"1>2\"></esi:when><esi:otherwise><esi:comment test=\"hello test text\" /> <esi:comment test=\"hello test text\" /> <esi:comment test=\"hello test text\" /> </esi:otherwise></esi:choose> AFTER")
	})
	const res = await makeRequest(url);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
...
test.todo("TEST 23b: Surrogate-Control removed when ESI enabled but no work needed (fast path)");
...
test("TEST 24: Fragment recursion limit", async () => {
	...
	routeHandler.add(`${url}/fragment_24`, function (req, res) {
		res.writeHead(200, esiHead);
...
test("TEST 24b: Lower fragment recursion limit", async () => {
	...
	routeHandler.add(`${url}/fragment_24b`, function (req, res) {
		res.writeHead(200, esiHead);
		res.say(`c: ${req.headers["x-esi-recursion-level"] || 0}`)
		res.end(`<esi:include src="${url}" />`)
	}, { count: 2 })
...
test("TEST 25: Multiple esi includes on a single line", async () => {
	...
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment_25a" /> <esi:include src="${url}/fragment_25b" />`)
	})
...
test("TEST 29: Remaining parent response chunks returned on fragment error", async () => {
	...
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n1\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n2`);
});

test.todo("TEST 30: Prime with ESI args - which should not enter cache key or reach the origin");
...
AFTER CONTENT`
	...
	const tests = [
		...
		{ url: `${url}?a=a&b=b`, responseString: `BEFORE CONTENT\na\nb\nAFTER CONTENT` },
		{ url: `${url}?l1d=l1d`, responseString: `BEFORE CONTENT\n\n\nAFTER CONTENT` },
		{ url: `${url}?c=c&l1d=l1d`, responseString: `BEFORE CONTENT\n\nc\n        l1d\n    \nAFTER CONTENT` },
		{ url: `${url}?c=c&l1e=l1e&l2f=l2f`, responseString: `BEFORE CONTENT\n\nc\n        l1e\n                l2f\n            \n    \nAFTER CONTENT` },
		{ url: `${url}?c=c&l1e=l1e`, responseString: `BEFORE CONTENT\n\nc\n        l1e\n                l2 OTHERWISE\n            \n    \nAFTER CONTENT` },
		{ url: `${url}?c=c`, responseString: `BEFORE CONTENT\n\nc\n        l1 OTHERWISE\n                \n            \n    \nAFTER CONTENT` },
		{ url: `${url}?c=c&l2g=l2g`, responseString: `BEFORE CONTENT\n\nc\n        l1 OTHERWISE\n                l2g\n            \n    \nAFTER CONTENT` },
	]
...
describe("TEST 31b: As above, no whitespace", () => {
	...
	const tests = [
		...
		{ url: `${url}?c=c&l1d=l1d`, responseString: `BEFORE CONTENTcl1dAFTER CONTENT` },
		{ url: `${url}?c=c&l1e=l1e&l2f=l2f`, responseString: `BEFORE CONTENTcl1el2fAFTER CONTENT` },
		{ url: `${url}?c=c&l1e=l1e`, responseString: `BEFORE CONTENTcl1el2 OTHERWISEAFTER CONTENT` },
		{ url: `${url}?c=c`, responseString: `BEFORE CONTENTcl1 OTHERWISEAFTER CONTENT` },
		{ url: `${url}?c=c&l2g=l2g`, responseString: `BEFORE CONTENTcl1 OTHERWISEl2gAFTER CONTENT` },
	]
...
test.todo("TEST 34: Leave instructions intact if surrogate-capability does not match http host");
...
test("TEST 37: SSRF", async () => {
	...
	routeHandler.add(`${url}?evil=foo%22/%3E%3Cesi:include%20src=%22/bad_frag%22%20/%3E`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment_1?$(QUERY_STRING{evil})" />`)
	})
	routeHandler.add(`${url}/fragment_1?foo%22/&gt;&lt;esi:include%20src=%22/bad_frag%22%20/&gt;`, function (req, res) {
		res.writeHead(200, esiHead)
		res.end(`FRAGMENT`)
	})
	const res = await makeRequest(`${url}?evil=foo"/><esi:include src="/bad_frag" />`);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`FRAGMENT`);
});
...
test("TEST 38: SSRF via <esi:vars>", async () => {
	...
	routeHandler.add(`${url}?evil=%3Cesi:include%20src=%22/bad_frag%22%20/%3E`, function (req, res) {
		res.writeHead(200, esiHead);
...
test("TEST 39: XSS via <esi:vars>", async () => {
	...
	routeHandler.add(`${url}?evil=%3Cscript%3Ealert(%22HAXXED%22);%3C/script%3E`, function (req, res) {
		res.writeHead(200, esiHead);
		res.say(`<esi:vars>$(QUERY_STRING{evil})</esi:vars>`)
		res.end(`<esi:vars>$(RAW_QUERY_STRING{evil})</esi:vars>`)
	})
	const res = await makeRequest(`${url}?evil=<script>alert("HAXXED");</script>`);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`&lt;script&gt;alert("HAXXED");&lt;/script&gt;\n<script>alert("HAXXED");</script>`);
});
...
Will never happen
	...
	routeHandler.add(`${url}?a=1&tag=foo%3Cscript%3Ealert(%22bad!%22)%3C/script%3Ebar`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(content)
	})
	const res = await makeRequest(`${url}?a=1&tag=foo<script>alert("bad!")</script>bar`);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
...
AFTER`
	routeHandler.add(`${url}?a=test&evil=%22%3Cesi:include%20src=%22/bad_frag%22%20/%3E`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(content)
	})
	routeHandler.add(`${url}/fragment_1?test=%22&lt;esi:include%20src=%22/bad_frag%22%20/&gt;`, function (req, res) {
		res.writeHead(200, esiHead);
		res.end('FRAGMENT')
	})
	const res = await makeRequest(`${url}?a=test&evil="<esi:include src="/bad_frag" />`);
	expect(res.ok).toBeTruthy()
	expect(checkSurrogate(res)).toBeTruthy()
	expect(await res.text()).toEqual(`BEFORE $(QUERY_STRING{a})\n\nFRAGMENT\ntest\n\nAFTER`);
});
...
test("TEST 46: Cookie var blacklist", async () => {
	...
	routeHandler.add(url, function (req, res) {
		...
		// And by key
		res.say(`<esi:vars>$(HTTP_COOKIE{allowed}):$(HTTP_COOKIE{not_allowed})</esi:vars>`)
		// ... and also in URIs
		res.end(`<esi:include src="${url}/fragment_1?&allowed=$(HTTP_COOKIE{allowed})&not_allowed=$(HTTP_COOKIE{not_allowed})" />`)
	});
	routeHandler.add(`${url}/fragment_1?&allowed=yes&not_allowed=`, function (req, res) {
		res.writeHead(200, esiHead);
		const url = new URL(req.url as string, testingDetails.url);
...
test("TEST 46b: Cookie var blacklist on fragment", async () => {
	...
	routeHandler.add(`${url}/fragment_1`, function (req, res) {
		...
		res.say(`<esi:vars>$(HTTP_COOKIE{allowed}):$(HTTP_COOKIE{not_allowed})</esi:vars>`)
		// ... and also in URIs
		res.end(`<esi:include src="${url}/fragment_2?&allowed=$(HTTP_COOKIE{allowed})&not_allowed=$(HTTP_COOKIE{not_allowed})" />`)
	});
	routeHandler.add(url, function (req, res) {
		res.writeHead(200, esiHead);
		res.end(`<esi:include src="${url}/fragment_1" />`);
	})
	routeHandler.add(`${url}/fragment_2?&allowed=yes&not_allowed=`, function (req, res) {
		res.writeHead(200, esiHead);
		const url = new URL(req.url as string, testingDetails.url);
		res.say(`FRAGMENT:${url.search}`)
		res.end(`${req.headers["cookie"]}`);

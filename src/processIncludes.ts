...
export async function process(eventData: ESIEventData, chunk: string, evalVars: boolean): Promise<string> {
...
async function fetchInclude(eventData: ESIEventData, include: string) {
	...
	if (sameDomain) {
		if (eventData.request.headers.get('Authorization')) {
			req.headers.set('Authorization', eventData.request.headers.get('Authorization') as string)
		}
		if (eventData.request.headers.get('Cookie')) {
			req.headers.set('Cookie', eventData.request.headers.get('Cookie') as string)
		}
	}

	// Add our recursion level and parent uri
	req.headers.set('X-ESI-Recursion-Level', (eventData.recursion + 1).toString())
	req.headers.set('X-ESI-Parent-URI', eventData.request.url)
	if (eventData.request.headers.get('Cache-Control')) {
		req.headers.set('Cache-Control', eventData.request.headers.get('Cache-Control') as string)
	}
...
function thirdPartyWhitelisted(config: ESIConfig, host: string): boolean {
	...
	if (config.disableThirdPartyIncludes) {
		if (!config.thirdPatyIncludesDomainWhitelist) { return false }
